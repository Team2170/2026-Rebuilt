...
public class Vision extends SubsystemBase {
    ...
    /**
     * Returns the X angle to the best target, which can be used for simple swerving
     * with vision.
     *
    ...
    // True if any camera can see at least one hub AprilTag 
    public boolean canSeeHubTags() {
        return getBestHubCamera() != -1;
    }

    /**
     * Horizontal angle error (degrees) to the midpoint between the two hub tags.
     * Positive = hub is left of center. Zero = robot is centered on hub opening.
     */
    public double getHubYawError() {
        int cam = getBestHubCamera();
        return cam == -1 ? 0.0 : inputs[cam].hubMidpointTx;
    }

    /** Returns index of the camera currently seeing hub tags, or -1 if none. */
    private int getBestHubCamera() {
        for (int i = 0; i < inputs.length; i++) {
            if (inputs[i].hubTagsVisible)
                return i;
        }
        return -1;
    }
